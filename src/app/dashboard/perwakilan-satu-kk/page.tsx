...
import { TambahPerwakilanModal } from "@/components/TambahPerwakilanModal";
...
type Stats = { total: number; todayCount: number };

const columnHelper = createColumnHelper<PerwakilanSatuKK>();

export default function PerwakilanSatuKKPage() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [ttdItem, setTtdItem] = useState<PerwakilanSatuKK | null>(null);
  const [deleteItem, setDeleteItem] = useState<PerwakilanSatuKK | null>(null);
  const [editItem, setEditItem] = useState<PerwakilanSatuKK | null>(null);
  ...
  const { data: statsData } = useQuery<Stats>({
    queryKey: ["perwakilan-satu-kk-stats"],
    queryFn: () => fetch("/api/stats-satu-kk").then(r => r.json()),
  });

  const { data, isLoading } = useQuery({
    queryKey: ["perwakilan-satu-kk", debouncedSearch, page],
    queryFn: () => {
      ...
      return fetch(`/api/perwakilan-satu-kk?${params}`).then(r => r.json());
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      fetch(`/api/perwakilan-satu-kk/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["perwakilan-satu-kk"] });
      queryClient.invalidateQueries({ queryKey: ["perwakilan-satu-kk-stats"] });
  ...
  const rows: PerwakilanSatuKK[] = data?.data ?? [];
  ...
  function downloadJson() {
    ...
    a.href = url;
    a.download = `perwakilan-1kk-${Date.now()}.json`;
    a.click();
  ...
  const columns = [
    ...
    columnHelper.accessor("ttdWakil", {
      ...
      cell: info => (
        <button
          onClick={() => setTtdItem(info.row.original)}
  ...
  const editItemAdapted = editItem ? {
    id: editItem.id,
    nomorUrut: editItem.nomorUrut,
    nik: editItem.nikWakil,
    namaLengkap: editItem.namaLengkapWakil,
  } : null;
    ...
    <div className="animate-fadeIn">
      <div className="mb-5">
        ...
        <p className="text-sm mt-0.5" style={{ color: "var(--text-secondary)" }}>
          Data perwakilan satu kepala keluarga
        </p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Cari nomor urut, nama wakil, atau NIK..."
              value={search}
          ...
          <div className="flex gap-2">
            <div className="relative">
              <button
                ...
                onClick={() => setShowAksiDropdown(!showAksiDropdown)}
          ...
          <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: "1px solid var(--border)" }}>
            ...
            <div className="flex gap-2">
              <button onClick={() => { setPage(p => Math.max(1, p - 1)); setRowSelection({}); }}
                disabled={page === 1} className="btn-secondary py-1 px-3 text-xs disabled:opacity-40">
                ← Prev
              </button>
              <button onClick={() => { setPage(p => Math.min(totalPages, p + 1)); setRowSelection({}); }}
                disabled={page === totalPages} className="btn-secondary py-1 px-3 text-xs disabled:opacity-40">
                Next →
              </button>
            </div>
          </div>
      ...
      {deleteItem && (
        <DeleteConfirmModal
          name={deleteItem.namaLengkapWakil}
            ...
            <div className="px-5 py-5 flex flex-col items-center text-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>
                  Data yang dipilih akan dihapus permanen dan tidak dapat dikembalikan.
                </p>
              </div>
            </div>
