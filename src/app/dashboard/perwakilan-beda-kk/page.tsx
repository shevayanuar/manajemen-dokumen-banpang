...
import { TambahBedaKKModal } from "@/components/TambahBedaKKModal";
import { EditBedaKKModal } from "@/components/EditBedaKKModal";
import { TandaTanganModal } from "@/components/TandaTanganModal";
import { DeleteConfirmModal } from "@/components/DeleteConfirmModal";
import { Modal } from "@/components/Modal";
import { useDebounce } from "@/hooks/useDebounce";

type PerwakilanBedaKK = {
  id: string;
  nomorUrut: number;
...
const columnHelper = createColumnHelper<PerwakilanBedaKK>();

export default function PerwakilanBedaKKPage() {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [ttdItem, setTtdItem] = useState<PerwakilanBedaKK | null>(null);
  const [deleteItem, setDeleteItem] = useState<PerwakilanBedaKK | null>(null);
  const [editItem, setEditItem] = useState<PerwakilanBedaKK | null>(null);
  ...
  const { data: statsData } = useQuery<{ total: number; todayCount: number }>({
    queryKey: ["perwakilan-beda-kk-stats"],
    queryFn: () => fetch("/api/stats-beda-kk").then(r => r.json()),
  });

  const { data, isLoading } = useQuery({
    queryKey: ["perwakilan-beda-kk", debouncedSearch, page],
    queryFn: () => {
      const params = new URLSearchParams({ search: debouncedSearch, page: String(page), limit: String(LIMIT) });
      return fetch(`/api/perwakilan-beda-kk?${params}`).then(r => r.json());
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => fetch(`/api/perwakilan-beda-kk/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["perwakilan-beda-kk"] });
      queryClient.invalidateQueries({ queryKey: ["perwakilan-beda-kk-stats"] });
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      await Promise.all(ids.map(id => fetch(`/api/perwakilan-beda-kk/${id}`, { method: "DELETE" })));
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["perwakilan-beda-kk"] });
      queryClient.invalidateQueries({ queryKey: ["perwakilan-beda-kk-stats"] });
  ...
  const rows: PerwakilanBedaKK[] = data?.data ?? [];
  ...
  function downloadJson() {
    const exportData = selectedRows.map(r => ({
      nomorUrutBarcode: r.nomorUrut,
      namaPenerimaBarcode: r.namaPenerimaBarcode,
      nikPenerimaBarcode: r.nikPenerimaBarcode,
      alamatPenerimaBarcode: r.alamatPenerimaBarcode,
      namaLengkapWakil: r.namaLengkapWakil,
      nikWakil: r.nikWakil,
      alamatWakil: r.alamatWakil,
    ...
    a.href = url;
    a.download = `perwakilan-beda-kk-${Date.now()}.json`;
    a.click();
  ...
  const columns = [
    ...
    columnHelper.accessor("namaPenerimaBarcode", {
      header: "Nama Penerima Barcode",
      cell: info => <span className="font-medium text-sm">{info.getValue()}</span>,
    }),
    columnHelper.accessor("nikPenerimaBarcode", {
      header: "NIK Penerima",
      cell: info => <span className="font-mono text-xs" style={{ color: "var(--text-secondary)" }}>{info.getValue()}</span>,
    }),
    columnHelper.accessor("alamatPenerimaBarcode", {
      header: "Alamat Penerima",
      cell: info => <span className="text-xs">{info.getValue()}</span>,
    }),
    columnHelper.accessor("namaLengkapWakil", {
      header: "Nama Lengkap Wakil",
      cell: info => <span className="font-medium text-sm">{info.getValue()}</span>,
    }),
    columnHelper.accessor("nikWakil", {
      header: "NIK Wakil",
      cell: info => <span className="font-mono text-xs" style={{ color: "var(--text-secondary)" }}>{info.getValue()}</span>,
    }),
    columnHelper.accessor("alamatWakil", {
      header: "Alamat Wakil",
      cell: info => <span className="text-xs">{info.getValue()}</span>,
    }),
    columnHelper.accessor("ttdWakil", {
      header: "TTD Wakil",
      cell: info => (
  ...
  const ttdItemAdapted = ttdItem ? {
    namaLengkap: ttdItem.namaLengkapWakil,
    nik: ttdItem.nikWakil,
    nomorUrut: ttdItem.nomorUrut,
    tandaTangan: ttdItem.ttdWakil,
  } : null;

  return (
    <div className="animate-fadeIn">
      <div className="mb-5">
        <h1 className="text-xl lg:text-2xl font-bold" style={{ color: "var(--text-primary)" }}>Perwakilan Beda KK</h1>
        <p className="text-sm mt-0.5" style={{ color: "var(--text-secondary)" }}>Data perwakilan beda kepala keluarga</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          <div className="relative flex-1">
            ...
            <input type="text" className="input-field pl-9"
              placeholder="Cari nomor urut, nama, atau NIK..."
              value={search}
            ...
            <div className="relative">
              ...
              {showAksiDropdown && hasSelection && (
                <>
                  ...
                  <div className="absolute left-0 top-full mt-1 w-52 rounded-xl overflow-hidden z-20 animate-scaleIn"
              ...
              {isLoading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i}>
                    {columns.map((_, j) => (
                      <td key={j}>
                        <div className="h-4 rounded animate-pulse" style={{ background: "var(--bg-card-hover)", width: "80%" }} />
                      </td>
                    ))}
                  </tr>
                ))
      ...
      {deleteItem && (
        <DeleteConfirmModal
          name={deleteItem.namaPenerimaBarcode}
